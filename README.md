...
Эксперимент с LLM-агентом в песочнице T-Invest API. Агент предлагает торговые
гипотезы, а Python-код проверяет ограничения, ведёт журнал и обрабатывает сбои.

Мне было интересно проверить, как собрать вокруг модели систему, поведение
которой можно восстановить и проверить: что произойдёт, если API не ответит,
агент забудет про позицию или бэктест покажет красивый результат на случайных
данных. Биржевая песочница даёт для этого конкретную задачу и виртуальный счёт.

**Это учебный инженерный эксперимент. Доходность не доказана, реальные деньги
не используются.** Боевой API не подключён. Прибыль — одна из метрик эксперимента,
а не обещание пользователю.
